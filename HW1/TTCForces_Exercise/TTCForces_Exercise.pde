...
TODO:
  1. Currently, there are no forces acting on the agents. Add a goal force that penalizes
     ...
     i.e., goal_force = goal_vel-current_vel; acc += goal_force DONE
  1a. Scale the goal force by the value k_goal. Find a value of k_goal that lets the agents
     ...
     i.e., goal_force = k_goal*(goal_vel-current_vel); acc += goal_force DONE
  2. Finish the function computeTTC() so that it computes the time-to-collision (TTC) between
     ...
     ie., TTC(A,B) = Ray-Disk(A's position, relative velocity, B's position, combined radius) DONE
...
Vec2 computeAgentForces(int id){
  ...
  Vec2 dir = goalPos[id].minus(agentPos[id]).normalized();
  Vec2 goal_vel = dir.times(goalSpeed);
  Vec2 goal_force = goal_vel.minus(agentVel[id]).times(k_goal); 
  
  acc = acc.plus(goal_force);
  ...
  for (int j = 0; j < numAgents; j++) {
    ...
    Vec2 af = agentPos[id].plus(agentVel[id].times(t));
    Vec2 bf = agentPos[j].plus(agentVel[j].times(t));
    Vec2 rel = (af.minus(bf)).normalized();
    acc = acc.plus(rel.times(k_avoid * (1 / t)));
  }
