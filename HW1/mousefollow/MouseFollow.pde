...
//     completely when it is nicely centered on the mouse. TODO
...
void update(float dt){
  ...
  Vec2 old_dis = mousePos.minus(pos);
  if (dir.length() > 0) dir.normalize();
  
  vel = dir.times(speed).times(1/frameRate);
  pos.add(vel);
  
  Vec2 new_dis = mousePos.minus(pos);
  
  if (old_dis.length() <= new_dis.length()) {
    pos = mousePos;
  }
}
...
void draw(){
  ...
  stroke(0,0,0);
  fill(255,0,0);
  //circle(pos.x, pos.y,r*2); //Question: Why is it radius*2 ? Third arg is extent/diameter, not radius.
